...
func Cors() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if origin != "" {
			...
			c.Header("Access-Control-Allow-Headers", "Authorization, Content-Length, X-CSRF-Token, token, wallet, session, X-Requested-With")
		...
		if method == "OPTIONS" {
			...
			c.Header("Access-Control-Allow-Headers", "Content-Type,AccessToken, X-CSRF-Token, Authorization, token, wallet, X-Requested-With") // custom Header
			c.Header("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
